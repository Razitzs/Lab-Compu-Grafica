...
#include <glfw3.h>
//glm
#include<glm.hpp> //las variables uniform sirven para no va a recibir por medio de vértices sino recibe el dato tal cual
...
int main()
{
	...
	//	6 cuadrado cafe
		...
		//techo
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 0.6f, -4.0f));//Coordenadas de la figura
		model = glm::scale(model, glm::vec3(0.6f, 0.35f, 1.0f));// Hacer la forma más pequeña (escalarla)
		...
		model = glm::scale(model, glm::vec3(0.35f, 0.45f, 1.0f)); // Hacer la forma más pequeña (escalarla)
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se envían al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		...
		model = glm::scale(model, glm::vec3(1.0f, 1.5f,1.0f)); // Hacer la forma más pequeña (escalarla)
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se envían al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		...
		model = glm::scale(model, glm::vec3(0.25f, 0.35f, 1.0f)); // Hacer la forma más pequeña (escalarla)
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se envían al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		...
		model = glm::scale(model, glm::vec3(0.2f, 0.3f, 1.0f)); // Hacer la forma más pequeña (escalarla)
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA y se envían al shader como variables de tipo uniform
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
...
//model = glm::translate(model, glm::vec3(0.5f, 0.0f, -4.0f));//Coordenadas de la figura
//model = glm::scale(model, glm::vec3(0.5f)); // Hacer la forma más pequeña (escalarla)
//meshColorList[4]->RenderMeshColor();
...
/*	model = glm::mat4(1.0);
	...
	//Aquí es donde se mandan a compilar las figuras 3d
	//Cubo
